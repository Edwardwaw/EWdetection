...
def cutout(image, pad_size, replace=0):
    ...
    Args:
        ...
        pad_size: Specifies how big the zero mask that will be generated is that
            is applied to the image. The mask will be of size (2*pad_size x 2*pad_size).
...
## translate image and corresponding gt boxes in X dimension or Y dimension --------------------------------------
...
def _shift_bbox(bbox, image_height, image_width, pixels, shift_horizontal):
    ...
    Args:
        ...
        shift_horizontal: Boolean. If true, shift in X dimension else shift in Y dimension.
...
## shear image and boxes in X dimension or Y dimension  -----------------------------------------------------------
...
def _cutout_inside_bbox(image, bbox, pad_fraction):
    ...
    Returns:
        A tuple.
        Fist element is a tensor of the same shape as image where each
        element is either a 1 or 0 that is used to determine where the image will have cutout applied.
        The second element is the mean of the pixels in the image where the bbox is located. mask value: [0,1]
    """
    image_height, image_width = image.shape[0], image.shape[1]
...
def distort_image_with_autoaugment(image, bboxes, augmentation_name):
    ...
    Args:
        image: shape [height, width, 3] representing an image.
        bboxes: shape [N, 4] representing ground truth boxes that are normalized between [0, 1].

        augmentation_name: The name of the AutoAugment policy to use. The available options are `v0`, `v1`, `v2`, `v3` and `test`.
            `v0` is the policy used for all of the results in the paper and was found to achieve the best results on the COCO dataset.
            `v1`, `v2` and `v3` are additional good policies found on the COCO dataset that have slight variation in what operations
            were used during the search procedure along with how many operations are applied in parallel to a single image (2 vs 3).
