...
def meet_emit_constraint(src_bbox, sample_bbox):
    '''
    if src center point belong to sample bbox, return True. else return false
    '''
...
def filter_and_process(sample_bbox, bboxes, labels, scores=None):
    ...
    for i in range(len(bboxes)):
        ...
        # if obj_bbox center point is not in sample_bbox or IoU==0, emit the obj_bbox
...
def generate_sample_bbox(sampler):
    # sampled_bbox=[xmin ymin xmax ymax]==> normalized [ltx,lty,rbx,rby]
...
def data_anchor_sampling(bbox_labels, image_width, image_height, scale_array, resize_width):
    '''
    以任意一个gt_box为中心，进行采样
    parameter:
    bbox_labels(list[array]): gt_bbox
    image_width:
    image_height:
    scale_array(list[float]): a list of anchor scales in data anchor sampling.
    resize_width(int): target image size.
    '''
    ...
    if num_gt != 0:
        ...
        for scale_ind in range(0, len(scale_array) - 1):
            if area > scale_array[scale_ind] ** 2 and area < scale_array[scale_ind + 1] ** 2:
        ...
        if rand_idx_size == range_size:
            ...
            max_resize_val = min(2.0 * scale_array[rand_idx_size], 2 * math.sqrt(wid * hei))
...
def jaccard_overlap(sample_bbox, object_bbox):
    '''
    return the IoU of one by one box
    parameter
    sample_box: shape=[4,]
    object_box: shape=[4,]
    '''
...
def intersect_bbox(bbox1, bbox2):
    '''
    return the intersection box of one by one box
    '''
...
def bbox_coverage(bbox1, bbox2):
    '''
    return  intersect_size(the intersect box of bbox1 and bbox2)/bbox1_size
    :param bbox1:
    :param bbox2:
    :return:
    '''
